...
static void eventq_get(CMD_T* cmd)
{
 ...
 // printf("Control set 1 !!!!!!!!!!");
	...
	//enable_fiq();				// is changed by TB
	enable_interrupt();			// is changed by TB
}
...
void Main(void)
{
	while (1)
	{
		if (eventq_get_count())					// is changed by TB
		{
...
//	mem_set_sram(&g_sata_context, 0, sizeof(g_sata_context));			// is changed by TB

//	g_sata_context.write_cache_enabled = TRUE;							// is changed by TB
...
//	while ((GETREG(PHY_DEBUG) & BIT30) == 1);							// is changed by TB
...
//	SETREG(APB_INT_STS, INTR_SATA);				// is changed by TB
...
//	SETREG(SATA_INT_STAT, 0x11000009);		// is changed by TB
//	SETREG(SATA_INT_ENABLE, 0x283);			// is changed by TB
