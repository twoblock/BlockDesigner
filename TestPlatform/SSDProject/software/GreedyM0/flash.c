...
void flash_issue_cmd(UINT32 const bank, UINT32 const sync)
{
	...
	//printf(" 3 bank %d Bank state 0x%x" , rbank,  _BSP_FSM(rbank) );
...
//		FLASH_TYPE == K9GBG08U0M || FLASH_TYPE == K9PFG08U5M || FLASH_TYPE == K9F1G08U0E
...
//
//	#else
//	//#error("ECC configuration error");			// is changed by TB
//	#endif
//
...
//
//	SETREG(WR_STAT, 0xFFFFFFFF);					// is changed by TB

	SETREG(BM_STACK_RDSET, 0);
	SETREG(BM_STACK_WRSET, 0);
	SETREG(BM_STACK_RESET, 0x03);
//	SETREG(FTL_READ_PTR, NUM_RD_BUFFERS + 1);		// is changed by TB

//	SETREG(REMAPBANK0, REMAP_BANK_INVALID);			// is changed by TB
//	SETREG(REMAPBANK1, REMAP_BANK_INVALID);
//	SETREG(REMAPBANK2, REMAP_BANK_INVALID);
//	SETREG(REMAPBANK3, REMAP_BANK_INVALID);

//	for (rbank = 0; rbank < NUM_BANKS_MAX; rbank++)	// is changed by TB
//	{
//		SETREG(_FCP_ROW_H(rbank), 0xFFFFFFFF);
//	}

//	for (bank = 0; bank < NUM_BANKS; bank++)		// is changed by TB
//	{
...
//	flash_finish();									// is changed by TB

//	flash_clear_irq();								// is changed by TB

//	#if FLASH_TYPE == MT29F64G08CFABA || FLASH_TYPE == MT29F128G08CJABA || FLASH_TYPE == JS29F32G08AAMDB	// is changed by TB
//
