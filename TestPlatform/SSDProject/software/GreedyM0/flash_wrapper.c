...
void nand_page_read(UINT32 const bank, UINT32 const vblock, UINT32 const page_num, UINT32 const buf_addr)
{
    ...
    SETREG(FCP_COL, 0);
	SETREG(FCP_ROW_L(bank), page_num);
	SETREG(FCP_ROW_H(bank), vblock);
...
void nand_page_program_from_host(UINT32 const bank, UINT32 const vblock, UINT32 const page_num)
{
    ...
    SETREG(FCP_COL, 0);
    //SETREG(FCP_ROW_L(bank), row);			// is changed by TB
    //SETREG(FCP_ROW_H(bank), row);			// is changed by TB
    SETREG(FCP_ROW_L(bank), page_num);
    SETREG(FCP_ROW_H(bank), vblock);
...
//    SETREG(FCP_ROW_L(bank), row);				// is changed by TB
//    SETREG(FCP_ROW_H(bank), row);				// is changed by TB
    SETREG(FCP_ROW_L(bank), page_num);			// is changed by TB
    SETREG(FCP_ROW_H(bank), vblock);			// is changed by TB
                          ...
                          UINT32 const dst_vblock, UINT32 const dst_page)
{
	...
	if (do_copyback)
	{
		...
		SETREG(FCP_ROW_L(bank), src_page);
		SETREG(FCP_ROW_H(bank), src_vblock);
		SETREG(FCP_DST_ROW_H, dst_vblock);
		SETREG(FCP_DST_ROW_L, dst_page);
	...
	else
	{
		...
		SETREG(FCP_ROW_L(bank), dst_page);
		SETREG(FCP_ROW_H(bank), dst_vblock);
		SETREG(FCP_BANK, REAL_BANK(bank));
		FLASH_POLL_WR;
		FLASH_ISSUE;
	}
