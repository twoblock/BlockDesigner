...
//	else if (cmd_type & ATR_LBA_EXT)
	if (cmd_type & ATR_LBA_EXT)				// is changed by TB
	{
...
//	else if (cmd_type & (CCL_FTL_H2D | CCL_FTL_D2H))
	if (cmd_type & (CCL_FTL_H2D | CCL_FTL_D2H))			// is changed by TB
	{
		UINT32 action_flags;
...
//#else
__irq void IRQ00_Handler(void)
//#endif
{
	//SETREG(SATA_INT_STAT, 0);
    SETREG(SATA_NCQ_BASE,0x0);             // nothing happen, there is no meaning
