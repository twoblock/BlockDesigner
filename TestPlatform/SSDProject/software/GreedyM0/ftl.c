...
//    g_ftl_statistics[bank].page_wcount++;
    ...
    //printf("vpn %d\n",PAGES_PER_BLK, new_vpn);
    set_vpn(lpn, new_vpn);
    //printf("vcount %d\n ",(get_vcount(bank, vblock) + 1));
    set_vcount(bank, vblock, get_vcount(bank, vblock) + 1);
}
...
/*     uart_printf("garbage_collection bank %d, vblock %d",bank, vt_vblock); */
	...
	//nand_page_read(bank, vt_vblock, PAGES_PER_BLK - 2, FTL_BUF(bank));
	...
	//printf("bank %d block %d page %d sector offset %d setor num %d", bank, vt_vblock, PAGES_PER_BLK - 1, 0,
                     //((sizeof(UINT32) * PAGES_PER_BLK + BYTES_PER_SECTOR - 1 ) / BYTES_PER_SECTOR), FTL_BUF(bank), RETURN_WHEN_DONE);
	...
	//printf("lpn_list_addr 0x%x size %x\n", g_misc_meta[bank].lpn_list_of_cur_vblock, sizeof(UINT32) * PAGES_PER_BLK);  
	
	//printf("lpn_list_addr %d size %d\n", g_misc_meta[bank].lpn_list_of_cur_vblock, sizeof(UINT32) * PAGES_PER_BLK);  
