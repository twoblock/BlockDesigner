	...
	public Set_BDPMD(ModuleInfo usedModuleDataList) {
		...
		for (int ModuleDataIndex = 0; ModuleDataIndex < usedModuleDataList.mList.size(); ModuleDataIndex++) {
			...
			if (ModuleDataIndex > 0) {
				obj_Channel_Connection_Info.put("module_name",
						usedModuleDataList.mList.get(ModuleDataIndex).module_instance_name);
				...
				obj_Channel_Item_HCLK.put("connection_num", String.valueOf(usedModuleDataList.mList.size() - 1));
				obj_Channel_Item_HCLK.put("name", usedModuleDataList.mList.get(0).module_instance_name + "$HCLK");
			...
			if (ModuleDataIndex > 0) {
				...
				obj_Channel_Item_HRESETn.put("connection_num", String.valueOf(usedModuleDataList.mList.size() - 1));
				obj_Channel_Item_HRESETn.put("name", usedModuleDataList.mList.get(0).module_instance_name + "$HRESETn");
			}
			...
			for (int PortIndex = 2; PortIndex < usedModuleDataList.mList.get(ModuleDataIndex).Port_List
				...
				String dModule = null;
				String dPort = null;
				String sModule = usedModuleDataList.mList.get(ModuleDataIndex).Port_List
						.get(PortIndex).Parent.module_instance_name;
				...
				if (usedModuleDataList.mList.get(ModuleDataIndex).Port_List.get(PortIndex).Dest_Port != null) {
					deportCheck = true;
					dModule = usedModuleDataList.mList.get(ModuleDataIndex).Port_List
							.get(PortIndex).Dest_Port.Parent.module_instance_name;
				...
				if (isUsed == false) {
					...
					if (sc_type.equals("sc_in") | sc_type.equals("sc_out")) {
					...
					} else if (data_type.equals("unsigned int")) {
						obj_Channel_Item.put("data_type", "UINT32");
					} else {
			...
			for (int ModulePortIndex = 0; ModulePortIndex < usedModuleDataList.mList.get(ModuleDataIndex).Port_List
				...
				if (usedModuleDataList.mList.get(ModuleDataIndex).Port_List.get(ModulePortIndex).addrSize != "0") {
					...
					obj_MemoryMap.put("slave_module", usedModuleDataList.mList.get(ModuleDataIndex).Port_List
							.get(ModulePortIndex).Dest_Port.Parent.module_instance_name);
					obj_MemoryMap.put("slave_module_port", usedModuleDataList.mList.get(ModuleDataIndex).Port_List
							.get(ModulePortIndex).Dest_Port.port_name);
